...
update_directory ()
{
  ...
  if (NULL == (rdc.buf = (char*)malloc (rdc.buf_len)))
    {
  ...
  response = (MHD_Response*)MHD_create_response_from_buffer (rdc.off,
					      rdc.buf,
...
process_upload_data (void *cls,
		     ...
		     size_t size)
{
  struct UploadContext *uc = (UploadContext*)cls;
  int i;
...
{
  struct UploadContext *uc = (UploadContext*)*con_cls;
	       ...
	       size_t *upload_data_size, void **ptr)
{
  ...
  if (0 == strcmp (method, MHD_HTTP_METHOD_POST))
    {
      /* upload! */
      struct UploadContext *uc = (UploadContext*)*ptr;

      if (NULL == uc)
	{
	  if (NULL == (uc = (UploadContext*)malloc (sizeof (struct UploadContext))))
...
main (int argc, char *const *argv)
{
  ...
  file_not_found_response = (MHD_Response*)MHD_create_response_from_buffer (strlen (FILE_NOT_FOUND_PAGE),
							     (void *) FILE_NOT_FOUND_PAGE,
							     MHD_RESPMEM_PERSISTENT);
  mark_as_html (file_not_found_response);
  request_refused_response = (MHD_Response*)MHD_create_response_from_buffer (strlen (REQUEST_REFUSED_PAGE),
							     (void *) REQUEST_REFUSED_PAGE,
							     MHD_RESPMEM_PERSISTENT);
  mark_as_html (request_refused_response);
  internal_error_response = (MHD_Response*)MHD_create_response_from_buffer (strlen (INTERNAL_ERROR_PAGE),
