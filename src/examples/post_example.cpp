	      ...
	      struct MHD_Connection *connection)
{
  ...
  reply = (char*)malloc (strlen (MAIN_PAGE) + strlen (session->value_1) + 1);
  if (NULL == reply)
    return MHD_NO;
  snprintf (reply,
		 ...
		 struct MHD_Connection *connection)
{
  ...
  reply = (char*)malloc (strlen (SECOND_PAGE) + strlen (session->value_1) + strlen (session->value_2) + 1);
  if (NULL == reply)
    return MHD_NO;
  snprintf (reply,
