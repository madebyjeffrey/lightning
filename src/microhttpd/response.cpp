		    ...
		    const char *content)
{
  ...
  if ( (NULL == response) ||
       ...
       (NULL != strchr (content, '\n')) )
    return MHD_NO;
  if (NULL == (hdr = reinterpret_cast<struct MHD_HTTP_Header *>(malloc (sizeof (struct MHD_HTTP_Header)))))
    return MHD_NO;
                                   ...
                                   MHD_ContentReaderFreeCallback crfc)
{
  ...
  if ((NULL == crc) || (0 == block_size))
    return NULL;
  if (NULL == (response = reinterpret_cast<struct MHD_Response *>(malloc (sizeof (struct MHD_Response) + block_size))))
    return NULL;
  memset (response, 0, sizeof (struct MHD_Response));
  response->fd = -1;
  response->data = reinterpret_cast<char*>(&response[1]);
...
{
  struct MHD_Response *response = reinterpret_cast<MHD_Response *>(cls);
  ssize_t n;
                               ...
                               void *data, int must_free, int must_copy)
{
  ...
  if ((NULL == data) && (size > 0))
    return NULL;
  if (NULL == (response = reinterpret_cast<MHD_Response *>(malloc (sizeof (struct MHD_Response)))))
    return NULL;
  memset (response, 0, sizeof (struct MHD_Response));
  response->fd = -1;
  if (MHD_YES != MHD_mutex_create_ (&response->mutex))
  ...
  response->reference_count = 1;
  response->total_size = size;
  response->data = reinterpret_cast<char*>(data);
