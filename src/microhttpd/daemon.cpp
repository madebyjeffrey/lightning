		  ...
		  socklen_t addrlen)
{
  ...
  if (NULL == (key = reinterpret_cast<struct MHD_IPCount *>(malloc (sizeof(*key)))))
    return MHD_NO;
  ...
  if (NULL == (nodep = reinterpret_cast<void **>(tsearch (key,
		  ...
		  socklen_t addrlen)
{
  ...
  /* Search for the IP address */
  if (NULL == (nodep = reinterpret_cast<void **>(tfind (&search_key,
			      &daemon->per_ip_connection_count,
			      &MHD_ip_addr_compare))))
    {
...
{
  struct MHD_Connection *con = reinterpret_cast<struct MHD_Connection *>(data);
  int num_ready;
  fd_set rs;
  fd_set ws;
  MHD_socket max;
			 ...
			 int external_add)
{
  ...
  if (NULL == (connection = reinterpret_cast<struct MHD_Connection *>(malloc (sizeof (struct MHD_Connection)))))
    {
      eno = errno;
#if HAVE_MESSAGES
      MHD_DLOG (daemon,
  ...
  if (NULL == (connection->addr = reinterpret_cast<struct sockaddr *>(malloc (addrlen))))
    {
      eno = errno;
#if HAVE_MESSAGES
      MHD_DLOG (daemon,
	...
	case MHD_OPTION_HTTPS_CRED_TYPE:
	  ...
	  break;
        case MHD_OPTION_HTTPS_MEM_DHPARAMS:
          if (0 != (daemon->options & MHD_USE_SSL))
            {
              const char *arg = reinterpret_cast<const char*>(va_arg (ap, const unsigned char *));
		     ...
		     va_list ap)
{
  ...
  if (daemon->nonce_nc_size > 0)
	   ...
	   sizeof(struct MHD_NonceNc) != daemon->nonce_nc_size)
	{
      ...
      daemon->nnc = reinterpret_cast<struct MHD_NonceNc *>(malloc (daemon->nonce_nc_size * sizeof (struct MHD_NonceNc)));
      if (NULL == daemon->nnc)
	{
#if HAVE_MESSAGES
	  MHD_DLOG (daemon,
