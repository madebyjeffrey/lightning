...
MHD_pool_create (size_t max)
{
  ...
  pool = reinterpret_cast<MemoryPool *>(malloc (sizeof (struct MemoryPool)));
  if (NULL == pool)
    return NULL;
#if defined(MAP_ANONYMOUS) || defined(_WIN32)
  if (max <= 32 * 1024)
    pool->memory = reinterpret_cast<char*>(MAP_FAILED);
  else
#if defined(MAP_ANONYMOUS) && !defined(_WIN32)
    pool->memory = reinterpret_cast<char*>(mmap (NULL, max, PROT_READ | PROT_WRITE,
  ...
  if ((pool->memory == MAP_FAILED) || (pool->memory == NULL))
    {
      pool->memory = reinterpret_cast<char*>(malloc (max));
      if (pool->memory == NULL)
        {
          free (pool);
          return NULL;
