                           ...
                           MHD_PostDataIterator iter, void *iter_cls)
{
  ...
  if (NULL == (ret = reinterpret_cast<struct MHD_PostProcessor *>(malloc (sizeof (struct MHD_PostProcessor) + buffer_size + 1))))
    return NULL;
...
{
  char *buf = (char *) &pp[1];
  /*const */ char *dash; // wtf was this const for?
  ...
  if ((0 != memcmp ("--", buf, 2)) || (0 != memcmp (&buf[2], boundary, blen)))
    {
      if (pp->state != PP_Init)
      ...
      else
        {
          ...
          dash = reinterpret_cast<char*>(memchr (buf, '-', pp->buffer_pos));
          if (NULL == dash)
  ...
  while (NULL != (spos = strstr (bpos, key)))
    {
      ...
      *destination = reinterpret_cast<char*>(malloc (vlen));
      if (NULL == *destination)
  ...
  while (1)
    {
      while (newline + 4 < pp->buffer_pos)
        {
          r = reinterpret_cast<char*>(memchr (&buf[newline], '\r', pp->buffer_pos - newline - 4));
          if (NULL == r)
          {
	  ...
	  pp->must_ikvi = MHD_YES;
        ...
        case PP_Nested_PerformMarking:
          ...
          if (pp->content_name != NULL)
            pp->have |= (NE_State)NE_content_name;
          if (pp->content_type != NULL)
