                          ...
                          const char *key, const char *value)
{
  ...
  pos = reinterpret_cast<MHD_HTTP_Header *>(MHD_pool_allocate (connection->pool,
      ...
      do
        {
          ...
          buf = reinterpret_cast<char*>(MHD_pool_allocate (connection->pool, size, MHD_NO));
        }
      while (NULL == buf);
...
try_grow_read_buffer (struct MHD_Connection *connection)
{
  ...
  connection->read_buffer = reinterpret_cast<char*>(buf);
...
build_header_response (struct MHD_Connection *connection)
{
  ...
  if (0 == strlen (connection->version))
    {
      data = reinterpret_cast<char*>(MHD_pool_allocate (connection->pool, 0, MHD_YES));
  ...
  /* produce data */
  data = reinterpret_cast<char*>(MHD_pool_allocate (connection->pool, size + 1, MHD_NO));
  if (NULL == data)
    {
#if HAVE_MESSAGES
			 ...
			 const char *message)
{
  ...
  response = (MHD_Response*)MHD_create_response_from_buffer (strlen (message),
					      (void *) message,
...
parse_cookie_header (struct MHD_Connection *connection)
{
  ...
  if (NULL == hdr)
    return MHD_YES;
  cpy = reinterpret_cast<char*>(MHD_pool_allocate (connection->pool, strlen (hdr) + 1, MHD_YES));
  if (NULL == cpy)
    {
#if HAVE_MESSAGES
  ...
  if ((line[0] == ' ') || (line[0] == '\t'))
    {
	 ...
	 ability to grow nicely. */
      last = reinterpret_cast<char*>(MHD_pool_reallocate (connection->pool,
                                  last,
                                  last_len + 1,
                                  last_len + tmp_len + 1));
      if (NULL == last)
                                     ...
                                     MHD_HTTP_HEADER_HOST)) )
    {
      ...
      response =
        MHD_create_response_from_buffer (strlen (REQUEST_LACKS_HOST),
					 (void*)REQUEST_LACKS_HOST,   // drakej -- evil fix
					 MHD_RESPMEM_PERSISTENT);
	    ...
	    daemon->notify_completed (daemon->notify_completed_cls,
          ...
          if (((MHD_YES == connection->read_closed) &&
          ...
          else
            {
              ...
              connection->read_buffer
                = reinterpret_cast<char*>(MHD_pool_reset (connection->pool,
                                  connection->read_buffer,
                                  connection->read_buffer_size));
            }
	  connection->client_aware = MHD_NO;
