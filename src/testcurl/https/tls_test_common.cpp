		 ...
		 int ver_peer)
{
  ...
  if (NULL == (cbc.buf = (char*)malloc (sizeof (char) * len)))
    {
          ...
          const char *version, size_t *upload_data_size, void **ptr)
{
  ...
  *ptr = NULL;                  /* reset when done */
  response = (MHD_Response*)MHD_create_response_from_buffer (strlen (test_data),
					      (void *) test_data,
...
gen_test_file_url (char *url, int port)
{
  ...
  if (NULL == (doc_path = (char*)malloc (doc_path_len)))
    {
      fprintf (stderr, MHD_E_MEM);
      return -1;
    }
	       ...
	       gnutls_certificate_credentials_t * xcred)
{
  ...
  cert->data = (unsigned char*)malloc (cert->size);
  if (NULL == cert->data)
    {
        gnutls_certificate_free_credentials (*xcred);
	free (key->data);
